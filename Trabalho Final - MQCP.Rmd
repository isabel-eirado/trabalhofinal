...
```{r paged.print=TRUE}
state_const_current_total_words %>% group_by(state) %>% summarise(ds_total_words=sd(total_words)) %>%
  ggplot(aes(x = constitution_year, 
             y = fct_inorder(ds_total_words),
             fill = state_abrev)) + #ao definir o mapping no ggplot, as orientações são globais para todos os outros gráficos que virem a seguir
  geom_col(alpha = 0.7, width = .65) + 
  scale_fill_brewer(palette = "Paired") +
  ggtitle("Variação do total de palavras das constituições por estado") +
  labs(x = "Ano da Constituição vigente",
       y = "Desvio padrão do total de palavras das constituições") +
  theme_minimal()
```
