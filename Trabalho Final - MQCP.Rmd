...
2. Verificar a tendência a mudança constitucional a partir do número de constituições implementadas ao longo do tempo
3. Observar, a partir de um índice, a mudança proporcional entre os três poderes nas constituições estaduais

- Contextualização

A organização do federalismo estadunidense dá às unidades subnacionais considerado potencial para inovações constitucionais e experimentação política ("espaço constitucional"), inclusive em comparação à maior parte de outras federações (BURGESS, TARR e MARKO, 2004).Essa possibilidade de mudança foi utilizada pelos estados de maneira frequente - seja pela substituição completa ou pelo emendamento das Cartas vigentes. Diferentemente do que ocorre com a Constituição Federal, emendas e mudanças nas Constituições Estaduais são relativamente fáceis, além de congregarem uma gama de atores, tendo um caráter popular mais visível, dado que são propostas ou votadas pela população (ACIR, 1989). 
...
4. current_constitution - informa a Constituição vigente (1) e as constituições históricas (0)
5. total_words - informa quantas palavras cada Constituição tem ou teve
6. legislative -  descreve as funções, procedimentos, poderes e limitações da legislatura
7. executive - descreve as funções, estrutura, poderes e limitações do Poder Executivo
8. judicial - descreve as funções e poderes dos sistemas de tribunais estaduais e dos funcionários do Poder Judiciário

Trabalhamos, portanto, com três variáveis categóricas, o nome dos estados, as abreviações e a variável "Constituição atual". As demais variáveis são numéricas e indicam desde o ano em que a Constituição foi promulgada (constitution_year) até o número de palavras dedicadas a determinado elemento em cada Constituição.
...
```{r mapa interativo, warning=FALSE, include=FALSE, echo=FALSE, warning=FALSE, message = FALSE}
