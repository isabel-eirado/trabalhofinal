...
1. A partir do Gráfico percebemos que Louisiana é o Estado com o maior número de constituições, enquanto diversos outros estados apresentam apenas duas mudanças constitucionais. São eles: 

```{r paged.print=TRUE, echo=FALSE, message = FALSE}
const_eua_painel %>% filter(numero_total_constituicoes ==2)
```

2. Enquanto a Califórnia figura como o estado com o maior desvio padrão entre as médias observadas nas estados, isto não se observa no quantitativo das mudanças constitucionais. O estado tem uma posição mediana, mudando suas normas constitucionais apenas 4 vezes. 
...
3. O histórico da quantidade de constituições já elaboradas no Estado ajuda a compreender as mudanças constitucionais, ao ponto em que alguns estados como Los Angeles tiveram um grande número de constituintes enquanto outros estados tiveram apenas duas. Para explorar melhor esses dados, sugerimos pesquisas qualitativas, a fim de identificar fatores políticos, econômicos e sociais que possam ter levado a essas diferenças no total de constituições de cada estado.
4. O ano de criação do estado aparenta não influenciar nas mudanças constitucionais.
