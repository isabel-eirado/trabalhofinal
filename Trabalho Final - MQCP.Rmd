...
#### **Mapa interativo das Constituições Estaduais dos Estados Unidos**

```{r ativando pac, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

library(leaflet) # Mapas interativos
library(tigris) # Dados para criação do mapa dos EUA
library(sf)
```
...
**H1**: *Há maior variação no número de palavras das Constituições Estaduais correntes, em relação às Constituições anteriores.*
 ...
 Além disso, foi utilizado um teste de correlação para verificar a relação entre *ano da Constituição* (constitution_year) e *número total de palavras*. Espera-se que haja um aumento do número de palavras ao longo do tempo.
